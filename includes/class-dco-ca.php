	...
	public function form_element( $type ) {
		...
		switch ( $type ) {
			...
			case 'autoembed-links-notification':
				_deprecated_argument( __FUNCTION__, esc_html__( 'WordPress Comment Attachment 2.0.0', 'dco-comment-attachment' ), esc_html__( 'The type "autoembed-links-notification" is deprecated. Use "autoembed-links" instead.', 'dco-comment-attachment' ) );
			...
			case 'autoembed-links':
				...
				if ( has_filter( 'dco_ca_form_element_autoembed_links_notification' ) ) {
					$markup = apply_filters_deprecated( 'dco_ca_form_element_autoembed_links_notification', array( ob_get_clean(), $autoembed_links ), 'WordPress Comment Attachment 2.0.0', 'dco_ca_form_element_autoembed_links' );
				} else {
